...
for i in range(0,num_train):
    ...
    x=samplesList_benign[idx_b]
    features=tf.train.Features(feature={
        'image/encoded': _bytes_feature(x.tostring()),
        'image/class/label': _int64_feature(0),
        'image/filename': _bytes_feature(str(namesList_benign[idx_b])),
    ...
    x=samplesList_malign[idx_m]
    feature={
        'image/encoded': _bytes_feature(x.tostring()),
        'image/class/label': _int64_feature(1),
        'image/filename': _bytes_feature(str(namesList_malign[idx_m])),
        'image/height': _int64_feature(MIN_DISTANCE_X),
        'image/width': _int64_feature(MIN_DISTANCE_Y),
        'image/channels': _int64_feature(CHANNELS),
    }
    ...
    writer.write(example.SerializeToString())
writer.close()
np.save('samplesListTrain',train)
np.save('namesListTrain',names_train)
np.save('labelsListTrain',labels_train)
...
for i in range(0,size[0]):#
    ...
    for j in range(0,sz[0]):
        y=x[j]
        samplesListTest.append(y)
        labelsListTest.append(label[i])
...
np.save('labelsListTest',labelsListTest)
np.save('namesListTest',namesListTest)
